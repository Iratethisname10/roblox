...
do
	...
	function library:Create(class, properties)
		...
		table.insert(self.instances, {object = inst, drawing = a})	;
		return inst;
	end;
	...
	function library:Unload()
		...
		for _, v in next, self.options do
			if v.type == 'toggle' and not string.find(string.lower(v.flag), 'panic') and v.flag ~= 'saveconfigauto' then
				pcall(v.SetState, v, false);
			end;
		end;
	...
	function library:LoadConfig(configName)
		...
		if (not configData) then print('[Library ]no config', configName); return; end;
	...
	do
		...
		task.spawn(function()
			...
			for _, v in next, Backgrounds do
